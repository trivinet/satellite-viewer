...
export default function InfoBoxPrint({setID,light,setInterval,setTotalPoints,setSelectedFam,setCenter,setMark,setViewMode,setViewTrace,setTleInfoShow,setIDfam, lng}) {
  ...
  const moveCenterToId = (id) => {
    const tle = assignTLE(id);
    const pts = Pos(tle, 1, intervalSaved); // may be '' on failure
    const p0 = Array.isArray(pts) && pts[0]
            && Number.isFinite(pts[0].lat) && Number.isFinite(pts[0].lng)
            ? pts[0]
            : null;
    if (p0) {
      setCenter([p0.lat, p0.lng]);
    }
    // else: silently skip — your TLE banner in map.js will handle messaging
  };    

  const safeLatLngForId = (id) => {
    try {
      const r = getLatLngObj(assignTLE(id), Date.now());
      return (r && Number.isFinite(r.lat) && Number.isFinite(r.lng)) ? r : null;
    } catch { return null; }
  };

  const safeSatInfoForId = (id) => {
    const ll = safeLatLngForId(id);
    if (!ll) return null;
    try {
      return getSatelliteInfo(assignTLE(id), Date.now(), ll.lat, ll.lng, 0);
    } catch { return null; }
  };

// SATÉLITES — sort by NORAD ID (numeric asc)
const activeSorted = React.useMemo(
  () => active.slice().sort((a, b) => (a.NORAD_CAT_ID ?? 0) - (b.NORAD_CAT_ID ?? 0)),
  [active]
);

// Reusable button factory
const makeItemButton = (item) => (
  <button
    key={item.NORAD_CAT_ID}
    onClick={() => {
      const id = item.NORAD_CAT_ID;
      setMark('');
      setSelectedFam(false);
      setSelectedFamInt(false);
      setTleInfoShow(true);
      setID(id);
      setIDsel(id);
      moveCenterToId(id); // safe centering
    }}
  >
    <a>{item.OBJECT_NAME}</a>
  </button>
);

// Initial list (memoized)
const itemsArchiveBuildInitial = React.useMemo(
  () => activeSorted.map(makeItemButton),
  [activeSorted]
);

const [itemsArchive, setItemsArchive] = useState(itemsArchiveBuildInitial);

// SATÉLITES INPUT (search by name prefix or ID prefix)
function handleChange(e) {
  const q = e.target.value.trim();
  if (!q) {
    setItemsArchive(itemsArchiveBuildInitial);
    return;
  }
  const qUpper = q.toUpperCase();
  const filtered = activeSorted.filter((it) =>
    it.OBJECT_NAME.toUpperCase().startsWith(qUpper) ||
    String(it.NORAD_CAT_ID).startsWith(q)
  );
  setItemsArchive(filtered.map(makeItemButton));
}
//FAMILIAS
    var familias = [];
      for (let i = 0; i < 10; i++) {
        familias=[(
          ...
          <button onClick={() => {setMark(''),setTleInfoShow(false),setSelectedFam(true),setSelectedFamInt(true)/* ,setTotalPoints(1) */,setID(IDScience),setIDsel(IDScience),setCenter([0,0])}}><a>{(lng=='ESP')?("Ciencia"):("Science")}</a></button>
        ),(
      ...
      return (
        <button
          ...
          onClick={() => {
            ...
            setIDfam(id);
            setMark(id);
            const pts = Pos(tle, 1, intervalSaved);
           const p0 =
             Array.isArray(pts) && pts[0] &&
             Number.isFinite(pts[0].lat) && Number.isFinite(pts[0].lng)
                ? pts[0] : null;
            if (p0) setCenter([p0.lat, p0.lng]);
          }}
        >
          <a>{name}, {id}</a>
        </button>
  ...
  return (
    ...
    <nav className={light?(styles.nav):(styles.navDark)}>
          <>
            ...
            {(() => {
              const ll = safeLatLngForId(IDsel);
              const si = safeSatInfoForId(IDsel);
              return (
                <>
                  <p style={{fontSize:'12px',marginLeft:'18px',padding:'5px'}}>{lng=='ESP'?("Latitud: "):("Latitude: " )}{ll ? ll.lat.toFixed(5) : 'n/a'} º</p>
                  <p style={{fontSize:'12px',marginLeft:'18px',padding:'5px'}}>{lng=='ESP'?("Longitud: "):("Longitude: " )}{ll ? ll.lng.toFixed(5) : 'n/a'} º</p>
                  <p style={{fontSize:'12px',marginLeft:'18px',padding:'5px'}}>{lng=='ESP'?("Altitud: "):("Altitude: " )}{si ? si.height.toFixed(2) : 'n/a'} km</p>
                  </>
                );
            })()}
            </>}</>)):(<>{<>
